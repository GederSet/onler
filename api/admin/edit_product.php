<?php
    ...
    $id_product = $admin->editProduct($id, $name, $price, $description, $weight, $water_tight, 
     ...
     for($i = 0; $i < count($img_array); $i++){
          ...
          if (!empty($img_array[$i])) {
               ...
               array_push($img_order, $i);
     

               // если мы получили картинку, то заносим её в папку с картинками
               if(strpos($img_array[$i]['type'], 'image/') === 0){
    ...
    $admin->deleteImages($id);

    sleep(1);
    $admin->editImages($img_name, $img_order, $id);
